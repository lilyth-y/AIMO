...
class Solver:
    ...
    def _init_model(self):
        try:
            model_name = os.environ.get('OMI_MODEL', 'Qwen/Qwen2.5-Math-72B-Instruct')
            quant = os.environ.get('OMI_QUANTIZATION')
...
def main():
    ...
    parser.add_argument('--model', type=str, help='HF model name to use (overrides OMI_MODEL)')
    parser.add_argument('--quant', type=str, help='Quantization setting (e.g., 4bit, 8bit) to use (overrides OMI_QUANTIZATION)')
    ...
    if args.model:
        os.environ['OMI_MODEL'] = args.model
    if args.quant:
        os.environ['OMI_QUANTIZATION'] = args.quant
    if args.cache_dir:
    ...
    print('[CONFIG] OMI_MODEL:', os.environ.get('OMI_MODEL'))
    print('[CONFIG] OMI_QUANTIZATION:', os.environ.get('OMI_QUANTIZATION'))
