...
class MoaiOrchestrator:
    ...
    def __init__(self, model_name=None, quantization=None, cache_dir=None):
        ...
        if model_name:
            os.environ['MATHCODEORCHESTRATOR_MODEL'] = str(model_name)
        if quantization:
            os.environ['MATHCODEORCHESTRATOR_QUANTIZATION'] = str(quantization)
        if cache_dir:
            os.environ['HF_HOME'] = str(cache_dir)
            os.environ['TRANSFORMERS_CACHE'] = str(cache_dir)

        print(f"[MoaiOrchestrator] Using model={os.environ.get('MATHCODEORCHESTRATOR_MODEL')} quant={os.environ.get('MATHCODEORCHESTRATOR_QUANTIZATION')} cache={os.environ.get('TRANSFORMERS_CACHE')}")
        try:
            import torch
        ...
        if os.environ.get('MATHCODEORCHESTRATOR_QUANTIZATION'):
            try:
