# AIMO Model Selection & Fine-tuning Guide

## 사용 가능한 모델

환경 변수 `AIMO_MODEL`로 모델 선택:

### 개발/테스트용
```powershell
$env:AIMO_MODEL="qwen-0.5b"  # 0.5B, 빠른 테스트용
```

### 권장 모델 (RTX 4060 8GB)
```powershell
$env:AIMO_MODEL="qwen-1.5b"  # 1.5B Coder (기본값, 권장)
$env:AIMO_MODEL="qwen-math-1.5b"  # 1.5B Math-specialized
```

### 큰 모델 (4-bit quantization)
```powershell
$env:AIMO_MODEL="qwen-7b"  # 7B Coder + 4-bit
$env:AIMO_MODEL="qwen-math-7b"  # 7B Math + 4-bit
$env:AIMO_MODEL="qwen-14b"  # 14B Coder + 4-bit (실험적)
```
...
5. **모델 평가**: AIMO reference problems로 벤치마크
