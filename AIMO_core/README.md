...
- Set environment variables to select a model and quantization format. Example:
    ```bash
    export AIMO_MODEL=qwen-1.5b
    export AIMO_QUANTIZATION=8bit  # or 4bit, or omit for full precision
    ```
    Or on Windows/PowerShell:
    ```powershell
    $env:AIMO_MODEL='qwen-1.5b'
    $env:AIMO_QUANTIZATION='8bit'
    ```
...
- Consider setting `AIMO_QUANTIZATION` to `8bit` or `4bit` to reduce memory usage. If you set `QUANTIZATION` to `8bit`/`4bit`, the `LocalLLMClient` will attempt to use BitsAndBytesConfig to load quantized models.
